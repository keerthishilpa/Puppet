...
if ruby --version > /dev/null 2>&1; then 
    ...
    sudo yum -y -e 0 -d 0 install ruby ruby-devel rubygems libselinux-ruby gcc make;
fi
...
if  ispresent "/usr/bin/lsb_release" ; then
    ...
    sudo yum -y -e 0 -d 0 install redhat-lsb-core;
    if  ispresent "/usr/bin/lsb_release" ; then
        print_systeminfo
    else
...
if ispresent "/usr/bin/rst2man" ; then
    ...
    sudo yum -y -e 0 -d 0 install python-docutils;
fi
...
mkdir -p /etc/puppet
cp conf/puppet.conf /etc/puppet/
if [[ $? -ne 0 ]]; then
    ret=$?
